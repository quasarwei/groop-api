...
tasksRouter.get('/:group_id', async (req, res, next) => {
  ...
  try {
    const groupTasks = await TasksService.getGroupTasks(req.app.get('db'), group_id);
...
tasksRouter.patch('/task/:task_id', jsonParser, async (req, res, next) => {
  ...
  const updateInfo = { name, description, date_due, completed, user_assigned_id };

  const numberOfValues = Object.values(updateInfo).filter(Boolean).length;
  if(numberOfValues == 0){
    return res.status(400)
      .json({error: {message: `Request must include at least one item to edit: name, description, date_due, completed, or user_assigned_id`}})
  }

  try {
    const updatedTask = await TasksService.updateTask( req.app.get('db'), task_id, updateInfo);
    res
      .status(200)
      .json(taskFormat(updatedTask));
  } catch (error) {
    next(error);
  }
});
...
tasksRouter.delete('/task/:task_id', async (req, res, next) => {
  const { task_id } = req.params;
  try{
    const deletedItem = await TasksService.deleteTask( req.app.get('db'), task_id);
    console.log(deletedItem);
    res.status(204)
      .json(`Task {task_id} was deleted.`)
  } catch (error) {
    next(error);
  }
});
